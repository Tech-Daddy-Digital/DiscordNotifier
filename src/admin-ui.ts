...
function normalizeRouteSelectorOptions(routes){return (routes||[]).filter((route)=>route&&route.id).map((route)=>({value:String(route.id),label:String(route.name||route.id)}));}
async function loadRouteSelectorOptions(guildId,fallbackRoutes=[]){try{const data=await api('/api/guilds/'+guildId+'/routes');return {options:normalizeRouteSelectorOptions(data.routes),error:null};}catch(error){return {options:normalizeRouteSelectorOptions(fallbackRoutes),error:error instanceof Error ? error.message : String(error)};}}
function routeOptions(routeSelectorOptions,currentValue){const value=currentValue || '';let options='<option value=""'+(value===''?' selected':'')+'>No route</option>';let found=value==='';for(const option of routeSelectorOptions||[]){const selected=option.value===value;if(selected) found=true;options+='<option value="'+h(option.value)+'"'+(selected?' selected':'')+'>'+h(option.label)+' ('+h(option.value)+')</option>';}if(value && !found){options+='<option value="'+h(value)+'" selected>'+h('Unknown saved ID: '+value)+'</option>';}return options;}
function routeOptionText(routeSelectorOptions,currentValue){const value=currentValue || '';if(!value)return 'No route';const option=(routeSelectorOptions||[]).find((entry)=>entry.value===value);return option ? option.label + ' (' + option.value + ')' : 'Unknown saved ID: ' + value;}
