...
function escapeClientHtml(value){return String(value ?? '').replace(/[&<>"']/g,function(char){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[char] || char;});}
function routeOptionLabel(route){return route.name + ' (' + route.id + ')';}
function routeSelect(routes){const options=['<option value="">No specific route</option>'].concat((routes || []).map(function(route){return '<option value="'+escapeClientHtml(route.id)+'">'+escapeClientHtml(routeOptionLabel(route))+'</option>';}));return '<select id="sourceRoute">'+options.join('')+'</select>';}
...
async function boot(){const data=await api('/api/guilds/'+guildId); const c=data.configuration; app.innerHTML='<div class="card"><h2>Settings</h2><label>Name<input id="guildName" value="'+escapeClientHtml(c.settings?.guildName||data.guild.name)+'"></label><label>Default channel ID<input id="defaultChannelId" value="'+escapeClientHtml(c.settings?.defaultChannelId||'')+'"></label><label>PulseDaddy admin role IDs, comma separated<input id="adminRoleIds" value="'+escapeClientHtml(c.adminRoleIds.join(', '))+'"></label><button onclick="saveSettings()">Save settings</button></div><div class="card"><h2>Discord channels</h2><pre>'+data.channels.map(ch=>escapeClientHtml(ch.name)+' — '+escapeClientHtml(ch.id)).join('\\n')+'</pre><h2>Roles</h2><pre>'+data.roles.map(r=>escapeClientHtml(r.name)+' — '+escapeClientHtml(r.id)).join('\\n')+'</pre></div><div class="card"><h2>Notification routes</h2>'+c.routes.map(r=>'<p><strong>'+escapeClientHtml(r.name)+'</strong> <code>'+escapeClientHtml(r.id)+'</code> → <code>'+escapeClientHtml(r.channelId)+'</code>, ping <code>'+escapeClientHtml(r.pingRoleId||'none')+'</code><br>'+escapeClientHtml(r.messageTemplate)+'</p>').join('')+'<input id="routeName" placeholder="Route name"><input id="routeChannel" placeholder="Discord channel ID"><input id="routePing" placeholder="Optional ping role ID"><textarea id="routeTemplate">{{displayName}} has a new {{type}} event: {{title}}</textarea><button onclick="saveRoute()">Add route</button></div><div class="card"><h2>Monitored sources</h2>'+c.sources.map(s=>'<p><strong>'+escapeClientHtml(s.displayName)+'</strong> ('+escapeClientHtml(s.type)+') <button onclick="delSource(\\''+escapeClientHtml(s.id)+'\\')">Delete</button></p>').join('')+'<select id="sourceType"><option>youtube</option><option>twitch</option><option>kick</option><option>x</option><option>bluesky</option><option>instagram</option></select><input id="sourceName" placeholder="Display name"><input id="sourceExternal" placeholder="Channel handle / account id"><input id="sourceUrl" placeholder="Optional URL">'+routeSelect(c.routes)+'<button onclick="saveSource()">Add source</button></div>';}
boot().catch(err=>{app.innerHTML='<pre>'+escapeClientHtml(err.message)+'</pre>'});
</script></body></html>`;
}
